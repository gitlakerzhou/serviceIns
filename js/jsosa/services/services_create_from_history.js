//var run_servicesCreateFrom = function() {
...
function services_edit_render_form () {
    ...
    divLeft.addTextInput("serviceCrtFromHistoryName", "srvc.name");
    divLeft.addTextNoInput("serviceCrtFromHistoryTenant", "srvc.tenant");
...
function services_edit_get_data() {
    holdTenantMap($("#serviceCrtFromHistoryTenant"));
...
(function (__soKey) {
    ...
    var getOptimizations = function(fields) {
        var opts = [];
        var vnfOpts = fields["vnf-optimization"];

        var cloudName = vnfOpts.cloudName || "";
        var aZone = vnfOpts.availabilityZone;
        var aggregate = vnfOpts.hostAggregate;
        var computeNode = vnfOpts.computeNode;
        var cloudLocationName = vnfOpts.cloudLocationName;
        
        var cloudID = vnfOpts.cloudKey || "0";
        var aZoneID = vnfOpts.availabilityZoneKey || "0";
        var aggregateID = vnfOpts.hostAggregateKey || "0";
        var computeNodeID = vnfOpts.computeNodeKey || "0";
        var cloudLocationID = vnfOpts.cloudLocationKey || "0";
    ...
    var processServiceOrderDesc = function(key) {
        ...
        // INITIAL POPULATION
        osa.ajax.get("serviceOrderHistory", key, function(soData) {
            var tenantInpt = $("#serviceCrtFromHistoryTenant");
            ...
            for (var i = 0; i < soData.vnfsCollection.length; i++) {
                ...
                var metadata = {
                    ...
                    "justCreated" : true // this allows the user to edit anything on the VNF.
                };
                ...
                for (var p = 0; p<custProps.length; p++) {
                    ...
                    var curCustProp = custProps[p];
                    metadata["property-" + curCustProp.vnfTypePropertyKey] = curCustProp.propertyValue;
                    vnfMetadataForLink["property-" + curCustProp.vnfTypePropertyKey] = curCustProp.propertyValue;
                }
            ...
            var getMetaData = function(key) {
                for (var i = 0; i < vnfMetadataForLink.length; i++) {
                    if (vnfMetadataForLink[i]["vnf-instance"] == key) {
            ...
            var getMetaDataByName = function(str) {
                for (var i = 0; i < vnfMetadataForLink.length; i++) {
                    if (vnfMetadataForLink[i]["vnf-instance-name"] == str) {
                        return vnfMetadataForLink[i];
                    }
                }
                return;
...
//                SOURCE
                ...
                // VNF
                    ...
                    var tmpKey = getVNFTypeByName($("#so-device--" + srcName.split(" ").join("-")).data("vnfType")).key;
                    srcMetaData = getMetaData(tmpKey);  
                    srcMetaData = getMetaDataByName(srcName);
...
//              DESTINATION
                ...
                // VNF
                    ...
                    var tmpKey = getVNFTypeByName($("#so-device--" + dstName.split(" ").join("-")).data("vnfType")).key;
                    dstMetaData = getMetaData(tmpKey); 
                    dstMetaData = getMetaDataByName(dstName);
    ...
    $("#edit_service_links_hdr .rightBtn").on("click", function(ev) {
        ...
        var tenantId = $("#serviceCrtFromHistoryTenant").data("tenantKey");
        var hasPhysDev = osa.auth.getPageAccess("physicalDevices").read;
        var isForCustomer = $("#serviceCrtFromHistoryTenant").data("tenantToTypeStrMap")[$("#serviceCrtFromHistoryTenant").html()] == "CUSTOMER";    // used to determine if site is required
    ...
    $("#service-edit-form").submit(function(ev) {
        ...
        if ($("#service-edit-form").valid()) {
            ...
            // -----------------------------------------------------------------
            so["ServiceOrderDesc"] = {
                    "@serviceName": $("#serviceCrtFromHistoryName").val(),
                    "@tenantName":  $("#serviceCrtFromHistoryTenant").html(),
                    "@tenantKey": $("#serviceCrtFromHistoryTenant").data("tenantKey"),
            ...
            $linkListContainer.find(".so-link-block").each(function(i, el) {
                ...
                if (linkData["src-type"] === "physical") {
                    ...
                    var portNames = linkData["src-metadata"]["vnf-portNames"] || linkData["src-metadata"].portNames;
                    ...
                    var vnfDataObj =$("#so-device--" + linkData["src-dev"]).data();
                ...
                if (linkData["dst-type"] === "physical") {
                    ...
                    var portNames = linkData["dst-metadata"]["vnf-portNames"] || linkData["dst-metadata"].portNames;
                    for (var i = 0; i < portNames.length; i++) {
                        cos[co].VNF[linkData["dst-dev"]].portCollection.push({key:i, soid: generateID(), displayName: portNames[i]});
                    }
                    //var vnfDataObj =$("#so-device--" + cos[co].VNF[linkData["src-dev"]]["vnf-instance-name"]).data();
                    var vnfDataObj =$("#so-device--" + linkData["dst-dev"]).data();
                ...
                if (Object.keys(cos[i].VNF).length > 0) {
                    ...
                    for (var j in cos[i].VNF) if (cos[i].VNF.hasOwnProperty(j)) {
                        ...
                        xmlSOCs[i].VirtualizedNetworkFunctions.VirtualizedNetworkFunction.push({
                            ...
                            "@customOptimization": curObj["vnf-optimization"] && curObj["vnf-optimization"].customOptimization || "",
